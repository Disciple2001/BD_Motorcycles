...
onMounted(() => {
  try {
    const promesa = window.electronAPI.get_modelos()
  ...
  try{
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        const pro = window.electronAPI.delete_modelo(id)
        pro.then(()=>{
          swalWithBootstrapButtons.fire({
            title: "Deleted!",
...
<!--            <button class="btn btn-outline btn-info btn-xs modal-button">details</button>-->
            ...
            <button class="btn btn-outline btn-error btn-xs" @click="handleDelete(item.id_modelo)">eliminar</button>
          </td>
        </tr>
      </tbody>
    </table>
